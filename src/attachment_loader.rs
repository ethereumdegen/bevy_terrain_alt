...
use bevy::{
    ...
    utils::HashMap, ecs::query::ReadOnlyWorldQuery,
};




pub(crate) struct AttachmentFromMemory {
    pub(crate) image: Handle<Image>,
    pub(crate) format: TextureFormat,
    pub(crate) file_format: FileFormat,
}

impl AttachmentFromMemory {
    pub(crate) fn new(attachment: &AttachmentConfig, image:Handle<Image>) -> Self {
        Self {
            image,
...
pub struct AttachmentFromMemoryLoader {
    pub(crate) attachments: HashMap<AttachmentIndex, AttachmentFromMemory>,
    /// Maps the id of an asset to the corresponding node id.
    handle_mapping: HashMap<HandleId, (NodeId, AttachmentIndex)>,
}
 
    
     pub(crate) fn start_loading_attachments_from_memory(
           asset_server: Res<AssetServer>,
           mut terrain_query: Query<(&mut NodeAtlas, &mut AttachmentFromMemoryLoader)>,
        ...
        let AttachmentFromMemoryLoader {
            ref mut attachments,
        ...
        for &node_id in load_events.iter() {
            ...
            ) in attachments.iter()
            {
                ...
                node.set_attachment(*attachment_index, handle.clone());
            }
        }
    }
...
pub struct AttachmentFromDiskLoader {
    ...
    handle_mapping: HashMap<HandleId, (NodeId, AttachmentIndex)>,
     ...
     pub(crate) fn start_loading_attachments_from_disk (
    asset_server: Res<AssetServer>,
    ...
    pub(crate) fn finish_loading_attachments (
